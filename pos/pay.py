...
def choice_age(guest_log) :
    ...
    return guest_log['age'] and False

# 물건 목록을 띄우고 그 중에서 어떤 물건을 구매할 지 선택
# 물건의 수량 정보까지 취합하여 저장
# 구매 물건으로 로또를 선택했을 경우 lotto.main()호출

def select_goods(tmp,goods,guest_log) :
    tmp_list = list(goods.keys())
    print("="*30)
    print("나이 : "+guest_log.get('age'))
    ...
    while True :
        ...
        if s_num in goods:                          #번호 기입이 상품번호 내에 있을 경우
...
def flow_choice(guest_log,goods) :
    ...
    while boolean :
        boolean = select_goods(tmp_goods,goods,guest_log)
